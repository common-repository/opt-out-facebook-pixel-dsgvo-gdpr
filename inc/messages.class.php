...
if ( ! class_exists( 'FBOO_Messages' ) ):

	class FBOO_Messages extends FBOO_Singleton {
		...
		private function add( $message, $type ) {
			...
			if ( ! $this->hasType( $type ) ) {
				$_SESSION[ FBOO_PREFIX ]['messages'][ $type ] = array();
			}

			if ( is_array( $message ) ) {
				$_SESSION[ FBOO_PREFIX ]['messages'][ $type ] = array_merge( $_SESSION[ FBOO_PREFIX ]['messages'][ $type ], $message );
			} else {
				$_SESSION[ FBOO_PREFIX ]['messages'][ $type ][] = $message;
			}
		...
		private function hasType( $type = null ) {
			if ( empty( $type ) ) {
				return ! empty( $_SESSION[ FBOO_PREFIX ]['messages'] );
			}

			return ! empty( $_SESSION[ FBOO_PREFIX ]['messages'][ $type ] );
		}
		...
		private function getMessage( $type = null ) {
			...
			if ( empty( $type ) ) {
				return $_SESSION[ FBOO_PREFIX ]['messages'];
			}

			return $_SESSION[ FBOO_PREFIX ]['messages'][ $type ];
		}
		...
		private function clearType( $type = null ) {
			if ( empty( $type ) ) {
				unset( $_SESSION[ FBOO_PREFIX ]['messages'] );

				return true;
			}

			unset( $_SESSION[ FBOO_PREFIX ]['messages'][ $type ] );
		...
		private function renderType( $type = null ) {
			...
			if ( is_null( $type ) ) {
				...
				foreach ( $messages as $type => &$messages ) {
					$html .= sprintf( $this->getMessageContainer(), $type . ' fboo-messages', $this->renderItems( $messages ) );
				}

			} else {
				// Render only specific type
				$html = sprintf( $this->getMessageContainer(), $type . ' fboo-messages', $this->renderItems( $this->getMessages( $type ) ) );
			}
